...
export default function SpreadsheetToolbar({
  ...
  return (
    <>
      ...
      <div style={{
      ...
      }} className="flex flex-col md:flex-row items-stretch md:items-center justify-between p-5 gap-4 border-b border-slate-200/60 dark:border-slate-700/40 bg-linear-to-r from-white via-slate-50/80 to-white dark:from-[#161920] dark:via-[#1A1E28]/60 dark:to-[#161920]">
        {/* Left toolbar group */}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div style={{
            backgroundColor: mp(theme, 'surface2'),
            borderColor: mp(theme, 'border'),
          }} className="flex items-center gap-px bg-slate-100 dark:bg-[#1E222A]/80 rounded-lg border border-slate-200/40 dark:border-slate-700/40">
            <label style={{ color: mp(theme, 'text') }} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-[#252A34] rounded-l-lg cursor-pointer transition">
            ...
            <div style={{ backgroundColor: mp(theme, 'border') }} className="w-px h-5 bg-slate-200/50 dark:bg-slate-700/50" />
            <button
              id="btn-export-csv"
              onClick={onExportCSV}
              style={{ color: mp(theme, 'text') }}
